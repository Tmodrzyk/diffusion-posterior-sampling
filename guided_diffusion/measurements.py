...
class UniformBlurOperator(LinearOperator):
    def __init__(self, device):
        self.device = device
        self.conv = Blurkernel(blur_type="uniform", kernel_size=9, device=device).to(
            device
        )
        self.kernel = self.conv.get_kernel()
        self.conv.update_weights(self.kernel.type(torch.float32))
    ...
    def get_kernel(self):
        return self.kernel.view(1, 1, 9, 9)


@register_operator(name="levin1_blur")
class Levin1BlurOperator(LinearOperator):
    def __init__(self, device):
        self.device = device
        self.conv = Blurkernel(blur_type="levin1", kernel_size=17, device=device).to(
            device
        )
        self.kernel = self.conv.get_kernel()
        self.conv.update_weights(self.kernel.type(torch.float32))
    ...
    def get_kernel(self):
        return self.kernel.view(1, 1, 17, 17)


@register_operator(name="levin4_blur")
class Levin4BlurOperator(LinearOperator):
    def __init__(self, device):
        self.device = device
        self.conv = Blurkernel(blur_type="levin4", kernel_size=13, device=device).to(
            device
        )
        self.kernel = self.conv.get_kernel()
        self.conv.update_weights(self.kernel.type(torch.float32))
    ...
    def get_kernel(self):
        return self.kernel.view(1, 1, 13, 13)
